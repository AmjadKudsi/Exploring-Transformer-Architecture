# add the proper .view() and .transpose() operations to reshape each tensor
import torch
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, mask=None):
        ...
        Q = self.w_q(query).view(batch_size, seq_len_q, self.num_heads, self.d_k).transpose(1, 2)  # TODO: Reshape for multi-head
        K = self.w_k(key).view(batch_size, seq_len_k, self.num_heads, self.d_k).transpose(1, 2)    # TODO: Reshape for multi-head
        V = self.w_v(value).view(batch_size, seq_len_k, self.num_heads, self.d_k).transpose(1, 2)  # TODO: Reshape for multi-head
